...
- Storage: Интеграция с Upstash Redis для хранения профилей

## Установка

1. Склонируйте этот репозиторий
2. Создайте новый проект на Vercel и подключите Upstash Redis
3. Добавьте переменные окружения `UPSTASH_REDIS_REST_URL` и `UPSTASH_REDIS_REST_TOKEN`
4. Выполните `vercel --prod`

## Как это работает?

Вместо стандартного TCP протокола, эта версия использует **WebSockets**. Vercel поддерживает WebSockets, что позволяет `frpc` (клиенту) подключаться напрямую к вашему домену на Vercel.
